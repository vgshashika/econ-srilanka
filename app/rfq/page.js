...
export default function RFQPage() {
  ...
  function validateStep(stepNumber) {
    if (stepNumber === 1) {
      if (!form.productName.trim() || !form.category || !form.quantity || Number(form.quantity) <= 0) {
        setError("Please complete product name, category, and a valid quantity.");
        return false;
      }
    }
    ...
    if (stepNumber === 3) {
      if (!form.fullName.trim() || !form.email.trim() || !form.country.trim()) {
        setError("Please complete your name, email, and country.");
        return false;
      }
    }
